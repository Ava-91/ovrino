...
export default function App() {
  ...
  useEffect(() => { void readHistory().then(setHistory); return () => { void ttsProvider.stop(); }; }, []);
  ...
  const handleGenerate = async () => {
    ...
    try {
      await ttsProvider.speak({ text: text.trim(), voice: toTtsVoice(selectedVoice), settings: { rate, pitch } });
      setGenerationState('idle');
      const item: SpeechHistoryItem = { id: `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, text: text.trim(), voiceId: selectedVoice.id, voiceName: selectedVoice.name, createdAt: new Date().toISOString(), rate, pitch };
  ...
  const removeHistory = async (id: string) => setHistory(await deleteHistoryItem(id));
  const removeAllHistory = async () => { await clearHistory(); setHistory([]); };
  ...
  return <SafeAreaView style={styles.safeArea}>
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.keyboardView}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}><View style={styles.sectionHeader}><Text style={styles.label}>TEXT</Text><Text style={styles.characterCount}>{text.length}/2,000</Text></View><TextInput multiline maxLength={2000} onChangeText={(value) => { setText(value); if (generationState === 'error') setGenerationState('idle'); }} placeholder="Write or paste something..." placeholderTextColor="#626978" style={styles.textInput} textAlignVertical="top" value={text} /></View>
        ...
        <HistoryPanel items={history} onSelect={restoreHistory} onDelete={removeHistory} onClear={removeAllHistory} />
